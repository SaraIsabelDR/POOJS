// Funcion sumArray: Suma de todos los elementos de un array
// Parametros: array (Array) - Un array de numeros
// Devuelve: Number - La suma de todos los elementos del array
const sumArray = (array) => {
    return array.reduce((Ac,Num) => Ac + Num , 0);
  };
  ...
  const findMaxNumber = (array) => {
    return array.find((Num) => Num === Math.max(...array));
  };
...
const agregarHabilidad = (jugador, nuevaHabilidad) => {
    ...
    */
    if ('habilidades' in jugador  && !(jugador.habilidades.includes(nuevaHabilidad))){
        jugador.habilidades.push(nuevaHabilidad);
    }else if (!('habilidades' in jugador)){
...
const calcularDuracionTotal = (peliculas) => {
  return peliculas.reduce((Ac,pelicula) => {
    if ('duracion' in pelicula && pelicula.duracion >= 0)
    {
      return Ac + pelicula.duracion
    }
    else{
      return Ac
    }  
...
const calcularPromedioPuntajes = (peliculas) => {
  const puntajes = peliculas.filter(pelicula => 'puntaje' in pelicula && pelicula.puntaje >= 0).map(pelicula => pelicula.puntaje);
  return puntajes.length > 0?calculateAverage(puntajes):0;
};
...
const calcularPromedioDuracionPorGenero = (peliculas, genero) => {
    ...
    const pelisflitradas = peliculas.filter(pelicula => (pelicula.genero).toLowerCase() === genero.toLowerCase())
    return pelisflitradas.length > 0?(calcularDuracionTotal(pelisflitradas) / pelisflitradas.length):0;
};
